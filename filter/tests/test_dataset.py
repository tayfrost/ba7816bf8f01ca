...
def test_dataset_item_format(sample_data, tokenizer):
    ...
    item = dataset[0]
    required_keys = ["input_ids", "attention_mask", "category_label", "severity_label", "is_risk"]
...
@pytest.mark.parametrize("idx,expected_cat,expected_sev", [
    (1, 0, 0),  # neutral, none
    (2, 6, 3),  # suicidal_ideation, late
    (0, 2, 1),  # stress, early
])
def test_dataset_label_encoding(sample_data, tokenizer, idx, expected_cat, expected_sev):
