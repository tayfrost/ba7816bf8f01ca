...
class StripeService:
    ...
    async def create_checkout_session(
        ...
        if not price_id:
            raise ValueError(
                f"Plan '{plan.name}' has no Stripe price configured for interval '{interval}'"
            )

        # Ensure Stripe customer exists
        customer_id = await StripeService.get_or_create_stripe_customer(db, org)
    ...
    async def cancel_subscription(
        ...
        sub = await StripeService.get_subscription(db, organization_id)
        if not sub:
...
async def _handle_checkout_completed(db: AsyncSession, session_obj: dict) -> None:
    ...
    if not (org_id and plan_id and stripe_sub_id):
