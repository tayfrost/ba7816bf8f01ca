...
DB_GRPC_HOST = os.getenv("DB_GRPC_HOST", "localhost:50051")


def _get_stub_or_skip():
    channel = grpc.insecure_channel(DB_GRPC_HOST)
    try:
        grpc.channel_ready_future(channel).result(timeout=1.5)
    except grpc.FutureTimeoutError:
        channel.close()
        pytest.skip(f"Database gRPC server not available on {DB_GRPC_HOST}")
    return db_pb2_grpc.DatabaseServiceStub(channel), channel
...
def test_full_grpc_flow():
    stub, channel = _get_stub_or_skip()
